...
namespace NetRewind.Utils
{
    public class ClientInputState : INetworkSerializable
    {
        ...
        private Vector2[] directionalInputs;
        ...
        private byte[] inputFlagsByte;
        private byte[] directionalInputsByte;
        ...
        public void NetworkSerialize<T>(BufferSerializer<T> serializer) where T : IReaderWriter
        {
            try
            {
                ...
                if (serializer.IsWriter)
                {
                    ...
                    serializer.SerializeValue(ref inputFlagsByte);
                    serializer.SerializeValue(ref directionalInputsByte);
        ...
        {
            int i = 0;
            inputFlags = new bool[InputFlags.Count];  // Initialize this only once

            foreach (var kvp in InputFlags)
            {
                inputFlags[i] = kvp.Value;
                i++;
            }

            int byteAmount = Mathf.CeilToInt(inputFlags.Length / 8f);
            byte[] byteArray = new byte[byteAmount];

            for (i = 0; i < inputFlags.Length; i++)
            {
                if (inputFlags[i]) // Only set bits for true values
                {
                    byteArray[i / 8] |= (byte)(1 << (i % 8));
                }
            }

            inputFlagsByte = byteArray;
        }
        
        private void DeserializeFlags()
        {
            int boolCount = InputCollector.NetworkInputFlagCount;
            ...
            {
                boolArray[i] = (inputFlagsByte[i / 8] & (1 << (i % 8))) != 0;
            }

            inputFlags = boolArray;

            i = 0;
            foreach (var input in inputFlags)
            {
                InputFlags.Add(InputCollector.GetNetworkInputFlagName(i), input);
                i++;
            }
        }
        ...
        private void SerializeDirectionals()
        {
            ...
            directionalInputsByte =  byteArray;
        }
        
        private void DeserializeDirectionals()
        {
            int vectorCount = InputCollector.NetworkDirectionalInputCount;
            ...
            {
                int shift = (i % 2) * 4; // Shift by 0 or 4 bits
                byte packedValue = (byte)((directionalInputsByte[i / 2] >> shift) & 0b1111); // Extract 4 bits
            ...
            directionalInputs = vectors;

            i = 0;
            foreach (var input in directionalInputs)
            {
                DirectionalInputs.Add(InputCollector.GetNetworkDirectionalInputName(i), input);
                i++;
            }
        }
